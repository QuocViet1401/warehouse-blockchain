...
router.get('/', requireLogin, async (req, res) => {
    const [orders] = await db.query(`
        SELECT i.*, p.name as product_name, p.code as product_code, u.fullname as created_by_name
        FROM import_orders i
        JOIN products p ON i.product_id = p.id
        JOIN users u ON i.created_by = u.id
        ORDER BY i.created_at DESC
    `);
    res.render('imports/index', { user: req.session.user, currentPath: '/imports', title: 'Nhập kho', orders });
});

router.get('/add', requireLogin, async (req, res) => {
    const [products] = await db.query('SELECT * FROM products ORDER BY name ASC');
    res.render('imports/form', { user: req.session.user, currentPath: '/imports', title: 'Tạo phiếu nhập', products, error: null });
});
...
router.post('/add', requireLogin, async (req, res) => {
    ...
    if (!product_id || !qty || qty <= 0) {
        const [products] = await db.query('SELECT * FROM products ORDER BY name ASC');
        return res.render('imports/form', { user: req.session.user, currentPath: '/imports', title: 'Tạo phiếu nhập', products, error: 'Dữ liệu không hợp lệ' });
    }
    const conn = await db.getConnection();
    try {
        await conn.beginTransaction();
        const [result] = await conn.query(
            'INSERT INTO import_orders (product_id, quantity, note, created_by) VALUES (?, ?, ?, ?)',
        ...
        await conn.rollback();
        const [products] = await db.query('SELECT * FROM products ORDER BY name ASC');
        res.render('imports/form', { user: req.session.user, currentPath: '/imports', title: 'Tạo phiếu nhập', products, error: err.message });
    } finally {
        conn.release();
    }
});
...
router.get('/:id', requireLogin, async (req, res) => {
    ...
    const [orders] = await db.query(`
        SELECT i.*, p.name as product_name, p.code as product_code, p.category,
        ...
        WHERE i.id = ?
    `, [req.params.id]);
    if (orders.length === 0) return res.redirect('/imports');

    const [blocks] = await db.query(
        "SELECT * FROM blockchain WHERE JSON_EXTRACT(data, '$.order_id') = ? AND JSON_EXTRACT(data, '$.transaction_type') = 'IMPORT'",
    ...
    res.render('imports/detail', {
        user: req.session.user, currentPath: '/imports',
        title: 'Phiếu nhập #' + req.params.id,
